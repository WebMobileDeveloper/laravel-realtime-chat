...
var app = new Vue({
    ...
    methods: {
        chat: function chat(_chat) {
            for (key in this.bArr) {
                if (key != _chat.id) {
                    this.chatBoxClose(key);
                    delete this.bArr[key];
                }
            }
            ...
            if (!this.chatBox.includes(_chat.id)) {
                ...
                // $('body').append(b.$el);
                $('#chat_box_container').append(b.$el);
...
socket.on('chatListRes', function (data) {
    ...
    if (app.chatLists.length > 0 && app.chatBox.length == 0) {
        app.chat(app.chatLists[0]);
    }
});
